...
//        mealsTo.forEach(System.out::println);
    ...
    public static List<UserMealWithExcess> filteredByCycles(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        ...
        for (var meal : meals) {
            calPerDay.merge(meal.getLocalDate(), meal.getCalories(), Integer::sum);
        }
        List<UserMealWithExcess> result = new ArrayList<>();
        for (var meal : meals) {
            if (TimeUtil.isBetweenHalfOpen(meal.getLocalTime(), startTime, endTime)) {
                result.add(new UserMealWithExcess(meal, calPerDay.get(meal.getLocalDate()) > caloriesPerDay));
            }
        }
        return result;
    }
    ...
    public static List<UserMealWithExcess> filteredByStreams(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        ...
        return meals.stream()
                ...
                .map(m -> new UserMealWithExcess(m, calPerDay.get(m.getLocalDate()) > caloriesPerDay))
